...
def limpiar_datos_credito(ruta_entrada, ruta_salida):
    ...
    if "EDUCATION" in df.columns:df = df[df["EDUCATION"] != 0]
    if "MARRIAGE" in df.columns:df = df[df["MARRIAGE"] != 0]

    df = df.dropna()

    if "EDUCATION" in df.columns:df.loc[df["EDUCATION"] > 4, "EDUCATION"] = 4
...
def crear_y_optimizar_pipeline(X_train, y_train):
    ...
    preprocessor = ColumnTransformer(
        ...
        remainder="passthrough",  # Mantener las variables numéricas intactas
    )
